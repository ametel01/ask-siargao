...
describe("Trip Pass checkout commerce", () => {
  ...
  test("hands a Session returned after Account Closure to durable expiry without exposing it", async () => {
    await withTestDb(async (db) => {
      ...
      const handoff = await db.query<{ status: string; step_status: string }>(
        `select s.status, step.status as step_status
         from account_closure_checkout_sessions s
         join account_closure_steps step on step.operation_id = s.operation_id
         where s.stripe_checkout_session_id = $1 and step.step_type = 'checkout_expiry'`,
        ["cs_order_checkout_closed_during_provider_call"],
      );
      expect(handoff.rows[0]).toEqual({ status: "pending", step_status: "pending" });
    });
  });
