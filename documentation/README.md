# Ask Siargao Documentation

This documentation set covers the public product and the people maintaining, operating, or
releasing the codebase.

- [User documentation](user/README.md)
- [Trip Pass reference](user/reference/trip-pass.md)
- [Developer documentation](developer/README.md)
- [First local run](developer/tutorials/first-local-run.md)
- [Build the data pipeline with local Postgres](developer/how-to-guides/build-the-data-pipeline-with-local-postgres.md)
- [Run Siargao field research](developer/how-to-guides/run-siargao-field-research.md)
- [Deterministic Field Workspace](developer/explanation/deterministic-field-workspace.md)
- [Recover Legacy Capture](developer/how-to-guides/recover-legacy-capture.md)
- [Release-candidate QA](developer/how-to-guides/run-release-candidate-qa.md)
- [Launch the free product](developer/how-to-guides/launch-free-product.md)
- [Launch Trip Pass](developer/how-to-guides/launch-trip-pass.md)
- [Environment reference](developer/reference/environment.md)
- [Clerk auth and account lifecycle](developer/reference/clerk-auth-session-chat-history-requirements.md)
- [Architecture and boundaries](developer/explanation/audit-lifecycle-and-boundaries.md)
- [Reality Check contract](developer/reference/reality-check-contract.md)
- [Planning guide analytics](developer/reference/planning-guide-analytics.md)
- [Field research protocol and data reference](developer/reference/field-research-data-model.md)
- [Siargao fieldwork official source pack](developer/reference/siargao-fieldwork-source-pack-2026-08-16.md)
- [Qualified Discovery strategy](developer/explanation/qualified-discovery-strategy.md)
- [On-demand Reality Check lifecycle](developer/explanation/on-demand-reality-check-lifecycle.md)
- [Extend a Reality Check kind](developer/how-to-guides/extend-a-reality-check-kind.md)
