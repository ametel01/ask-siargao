...
| DeepSeek | Chat model integration retained but disabled when `CHAT_MODEL_PROVIDER=openai` | China/provider-managed | No production data while disabled. If enabled: traveler prompt and context after application minimization, generated answer, token and cost metadata | The application does not durably log raw prompt text, but provider retention/deletion terms for downstream API data are not evidenced | [Open Platform terms](https://cdn.deepseek.com/policies/en-US/deepseek-open-platform-terms-of-service.html) and [privacy policy](https://cdn.deepseek.com/policies/en-US/deepseek-privacy-policy.html). No executed DPA or complete subprocessor register is on file. | Conditional: prohibited for production until privacy/legal and DPA-equivalent evidence exist |
...
1. Obtain explicit approval for the paid OpenAI primary model and configure its USD 10/day
   application cap. If DeepSeek becomes the selected provider, resolve its downstream API privacy
   and DPA gap before enabling it.
2. Deliver production secrets through the restricted 1Password-to-Vercel workflow and verify that
   disabled vendors have no production keys.
