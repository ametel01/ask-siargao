...
This register is the production data-flow and supplier inventory for Ask Siargao as of 2026-08-12.
...
| PostHog | US Cloud product analytics; disabled for the initial production launch; IP anonymization and autocapture opt-out are configured; session replay disabled | US Cloud | No production data while `NEXT_PUBLIC_POSTHOG_KEY` is unset. If later enabled: allowlisted product events and pseudonymous identifiers only; no prompts or message text | Project retention remains 12 months (`event_retention_months=12`); no production events are sent during the initial launch; delete persons/events/project through PostHog | [Terms](https://posthog.com/terms), [DPA](https://posthog.com/dpa), and [subprocessors](https://posthog.com/subprocessors). | Conditional: disabled; reduce retention to 90 days or approve a variance before enabling |
| DeepSeek | Owner-selected primary chat model for the proposed controlled beta; production exposure remains off | China/provider-managed | Traveler prompt and limited recent chat context after application minimization, generated answer, token and cost metadata. The app does not intentionally send account email, payment data, or raw browser coordinates to the model. | Ask Siargao does not durably log raw prompt text. DeepSeek's published policy does not state a fixed downstream API retention period, says input may be kept while the provider account exists, and documents default on-disk context caching. | [Open Platform terms](https://cdn.deepseek.com/policies/en-US/deepseek-open-platform-terms-of-service.html), [privacy policy](https://cdn.deepseek.com/policies/en-US/deepseek-privacy-policy.html), and [context-caching documentation](https://api-docs.deepseek.com/guides/kv_cache/). No executed DPA or complete public subprocessor register is on file. | Conditional: user disclosure and versioned acknowledgement are implemented; exposure still requires the named owner to record an explicit privacy/legal variance accepting the documented DPA, subprocessor, retention, training-use, caching, and China-transfer gaps |
...
| Open-Meteo | Weather and marine adapters retained for local noncommercial development; disabled for the initial production launch | Switzerland/provider-managed API | No production data while `OPEN_METEO_API_MODE=off`. Local development transmits coordinates, server IP, and request metadata; no account identity | Application retention follows source-governance rules; Open-Meteo says individual API logs are deleted after 90 days | [Terms and privacy](https://open-meteo.com/en/terms). Free API use is non-commercial only; paid customer endpoints and credentials require a separate adapter. Production validation rejects noncommercial mode, scheduled calls are absent from `vercel.json`, and retained routes return no-ops. | Conditional: disabled; implement a paid commercial adapter before enabling in production |
...
| --- | --- | --- | --- | --- | --- |
| OpenAI | Candidate primary chat provider using `gpt-5.4-mini`; hosted memory and web research remain disabled | Provider-managed/global; minimized prompts, outputs, and API metadata if enabled | API Customer Data is normally retained for no more than 30 days; no files or vector stores are required by the initial production path | [Services Agreement](https://openai.com/policies/services-agreement/), [DPA](https://openai.com/policies/data-processing-addendum/), and [subprocessors](https://openai.com/policies/sub-processor-list/). | Conditional: requires explicit paid-provider approval; configure a documented USD 10/day application cap before enabling |
| Tide-Forecast / Meteo365 | Dapa tide page adapter retained for local development only; disabled for production | Provider-managed; no production data while `TIDE_FORECAST_MODE=off` | No production calls or retained production payloads while disabled | [Terms](https://www.tide-forecast.com/pages/terms). The current HTML adapter has no commercial license record. Production validation rejects development mode. | Conditional: disabled; obtain permission/license and implement an approved adapter before enabling |
...
| Tide-Forecast / Meteo365 | No production account or license | Stop calls and remove locally retained development data | Contact route in the provider terms |
...
1. Record the named owner's explicit DeepSeek privacy/legal variance after review of the public
   notice and current provider terms. The decision must explicitly accept the missing DPA and
   subprocessor evidence, indefinite downstream API retention language, provider training use,
   default context caching, and processing/storage in China.
2. Verify the versioned browser acknowledgement and production `428` fail-closed boundary on the
   exact deployment. Production runtime secrets were generated into the restricted 1Password vault
   and delivered to Vercel on 2026-08-13 without printing their values.
3. Execute or retain evidence of the Sentry DPA before any event is permitted to contain Personal
   Data; the initial scrubbed-only event policy remains in force.
