...
| `GOOGLE_API_KEY` | Server only | Google Places discovery and enrichment | Required by `bun run db:discover:google-places` and `bun run db:enrich:google-places`. Keep field masks narrow; discovery uses ID-only fields, enrichment uses Place Details Pro fields. |
