...
| `CHAT_MODEL_PROVIDER` | Server only | Production chat provider selection | Optional enum: `deepseek` or `openai`; defaults to `deepseek`. Production startup requires the selected provider's API key. Select `openai` for the initial launch while the DeepSeek privacy/legal gate is open. |
| `DEEPSEEK_API_KEY` | Server only | DeepSeek Ask Siargao chat model | Required when `CHAT_MODEL_PROVIDER=deepseek`. DeepSeek must remain disabled in production while its privacy/legal gate is open. |
