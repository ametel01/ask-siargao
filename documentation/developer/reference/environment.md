...
| `CLERK_PRODUCTION_VERCEL_URL` | Server only | Production platform binding | Exact platform-provided `VERCEL_URL` host for production. Required when `CLERK_DEPLOYMENT_CONTEXT=production`; mismatched `VERCEL_ENV` or `VERCEL_URL` fails deployment validation. |
...
| `CLERK_PROTECTED_STAGING_VERCEL_URL` | Server only | Protected staging platform binding | Exact platform-provided `VERCEL_URL` host for the stable protected staging deployment. Required when `CLERK_DEPLOYMENT_CONTEXT=protected-staging`; ordinary preview deployment URLs are rejected even if they claim staging context. |
...
| `PLAYWRIGHT_PROTECTED_UI_HARNESS` | Server only | Local E2E protected-page UI harness | Test-only. When set to `1` in local/test disabled Clerk mode, Playwright can send `x-ask-siargao-protected-ui-harness: 1` to exercise protected page UI without disabling the default 404 perimeter. Never set in deployed environments. |
