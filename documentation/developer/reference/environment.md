...
| `PROVIDER_RC_CLERK_GOOGLE_USER` | Protected CI secret | Clerk lane | Dedicated `+clerk_test` account with one verified Google external account. The lane proves Google OAuth is offered and the linked identity remains verified without storing Google credentials. |
