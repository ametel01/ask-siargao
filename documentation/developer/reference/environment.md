...
| `FIELD_RESEARCHER_ACCOUNT_IDS` | Server only | Field Workspace authorization | Comma-separated immutable Clerk Account IDs authorized as Field Researchers. Operator accounts are also accepted through `OPERATOR_ACCOUNT_IDS`; neither allowlist is sent to clients. |
| `FIELD_WEBAUTHN_CHALLENGE_SECRET` | Server only | Authorized Field Device registration | At least 32 bytes used to authenticate short-lived same-site registration challenge cookies. It is not a Field Recovery Secret and must not be reused for grant signing. |
| `FIELD_GRANT_SIGNER_KEY_ID` | Server only | Offline Field Grant issuance | Versioned identifier for the active P-256 grant signer. Retain prior public keys until every grant they signed has expired. |
| `FIELD_GRANT_SIGNING_PRIVATE_JWK` | Server only | Offline Field Grant issuance | P-256 private JWK held only by the server secret store. Never use a public-prefixed variable or include it in evidence. |
| `FIELD_GRANT_SIGNING_PUBLIC_JWK` | Server only response input | Offline Field Grant verification | Matching public P-256 JWK delivered during explicit online grant issuance and pinned with the encrypted local authorization state. |
| `NEXT_PUBLIC_FIELD_CACHE_GENERATION` | Public/client-safe | Field Workspace service-worker cache pin | Immutable deployment/build identifier used to keep an active Visit on one shell/chunk generation. It contains no identity or field values. |
| `FIELD_PLANNER_INITIAL_HANDOFF_JSON` | Server only | Clean-device Field Planner first-run handoff | Optional machine-generated, protocol-pinned `FieldPlannerReadinessHandoff` JSON pre-seeded by the authorized Field operator. The authenticated planning endpoint serves it to an allowlisted researcher; it is never a browser authoring surface. Rotate it when the protocol or campaign evidence changes, and leave it unset to fail closed. |
| `FIELD_LEGACY_IMPORT_ALIAS_MODE` | Server only | Temporary Legacy Capture route rollback | Optional strict mode: `redirect` (default, including malformed or missing values) sends `/admin/field-ingestion` to the canonical Diagnostics and Recovery route with a temporary 307; `fallback` renders the same Field Researcher-authorized, device-locked compatibility component at the old URL. It never restores admin-token authorization or old workbench actions. |
