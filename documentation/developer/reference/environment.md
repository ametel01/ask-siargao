...
| `PROVIDER_RC_CLERK_GOOGLE_EMAIL` | Protected CI secret | Clerk lane | Dedicated challenge-free Google test account used to complete the configured Clerk OAuth redirect and callback. Never reuse a personal or production identity. |
| `PROVIDER_RC_CLERK_GOOGLE_PASSWORD` | Protected CI secret | Clerk lane | Password for the dedicated Google test account. Keep step-scoped in the protected environment; interactive challenge, CAPTCHA, or 2FA fails the lane closed. |
...
| `MODEL_COST_RESERVATION_MICRO_USD` | Server only | Model cost circuit reservation size | Optional positive integer reservation in micro-USD per model call. Defaults to `2000`; DeepSeek reservations reconcile downward or upward to modeled provider usage after each call. |
...
| `TRAVEL_ANSWER_EXPOSURE_MODE` | Server only | Emergency and staffed exposure control | Production accepts new Travel Answers only when exactly `staffed`; missing, malformed, or `off` closes exposure. The server also enforces 08:00–22:00 PHT and the 1,000-answer UTC-day cap. Set `off` for emergency shutdown. |
...
| `SENTRY_CRON_MONITOR_SLUG` | Server only | Quota-efficient Cron monitoring | Optional outside production; defaults to `ask-siargao-account-closure`. Set the same slug in staging and production so `/api/cron/operations` emits the single aggregate check-in allowed by the Sentry Developer plan. Weather, marine, and Places pruning remain independently tracked in `operational_schedule_states` and create scrubbed Sentry events when failed or stale. |
