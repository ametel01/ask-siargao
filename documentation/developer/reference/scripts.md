...
| `bun run test:smoke:trip-pass-stripe` | `bun run src/server/qa/run-stripe-release-candidate.ts` | Run authenticated protected-app Playwright against real Stripe test mode: exact deployed-SHA proof, Checkout retry/return/expiry/payment, signed webhooks, activation/duplicate/reversed delivery, paid-answer settlement, cumulative refunds, dispute, closure race, and Paid After Closure; then run the supplemental lifecycle/order contracts. |
