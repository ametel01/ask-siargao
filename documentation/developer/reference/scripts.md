...
| `bun run field-protocol:generate` | `bun run src/features/field-protocol/generate-field-protocol.ts` | Regenerate TypeScript record bindings, typed Observation Kind values, and the embedded baseline package from the signed canonical artifacts. Signing a changed package additionally requires the uncommitted release private key through `--sign-private-key=<path>`; the repository contains only its trusted public key. |
| `bun run field-protocol:check` | `bun run src/features/field-protocol/generate-field-protocol.ts --check` | Fail when generated Field Protocol Package bindings are missing or stale, or when a manifest content hash does not match its canonical artifact. The Bun test suite runs this check. |
