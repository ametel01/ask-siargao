...
| `bun run operations:worker` | `bun run src/server/operations/run-operational-worker.ts` | Process durable provider-neutral Account Closure, pending payment receipts, Lemon Squeezy refund operations, retained Stripe evidence, retention purge, and reconciliation tasks through production handlers. Use `-- --task=<kind-or-all>`, `--batch=<positive-count>`, and `--lease-seconds=<positive-count>`; task claims, retries, crash recovery, and completion are database-time lease fenced. This command does not choose a scheduler vendor or cadence. |
