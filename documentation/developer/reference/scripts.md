...
| `bun run build` | `bun run validate:deployment && rm -rf .next && NEXT_PRIVATE_BUILD_WORKER=0 ./node_modules/.bin/next build` | Reject invalid Clerk, selected chat provider, shared production model-cost circuit, forecast-provider, and Trip Pass checkout configuration, then build the production Next.js app from a clean Next output directory. |
